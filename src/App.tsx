Warning: truncated output (original token count: 55761)
Total output lines: 4463
                                ...
                                <div className="absolute right-0 top-10 z-40 w-52 rounded-xl border border-slate-100 bg-white py-1 text-left text-[11px] font-semibold text-slate-600 shadow-2xl">
                                  {singleSelectedDoc && (
                                    <>
                                      ...
                                      <button onClick={(e) => { handleMakeCopy(singleSelectedDoc.id, e); setOpenMenuId(null); }} className…5761 tokens truncated… on staff units. Click explore to browse scoped folder registries.</p>
              </div>
      ...
      {selectedDocId && docDetail && (
        <>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            {/* ACTIVE SHARED SECURE ACCESS LINKS (WeTransfer-style): collapsed by
            ...
            {docDetail.externalLinks && docDetail.externalLinks.length > 0 && (
              <div className="space-y-2">
                <button
                  type="button"
                  onClick={() => setShowShareLinks(s => !s)}
                  className="w-full flex justify-between items-center"
                >
                  <span className="text-[10px] font-mono font-bold tracking-wider text-slate-400 uppercase flex items-center space-x-1.5">
                    <Link2 className="w-3 h-3 text-slate-400" />
                    <span>Active Share Links</span>
                    <span className="text-slate-300 normal-case font-sans">({docDetail.externalLinks.length})</span>
                  </span>
                  <ChevronRight className={`w-3.5 h-3.5 text-slate-400 transition-transform shrink-0 ${showShareLinks ? 'rotate-90' : ''}`} />
                </button>

                {showShareLinks && (
                  <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-2 space-y-1.5">
                    {docDetail.externalLinks.map(link => {
                      ...
                      return (
                        <div
                          key={link.id}
                          title={link.message || undefined}
                          className={`flex items-center justify-between gap-2 px-2.5 py-2 bg-white rounded-lg border ${isExpired || isExhausted ? 'border-rose-100 opacity-60' : 'border-slate-100'}`}
                        >
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center space-x-1 min-w-0">
                              <p className="text-[10px] font-semibold text-slate-700 truncate" title={link.fileName}>{link.fileName || link.token}</p>
                              {link.requiresPassword && <Lock className="w-2.5 h-2.5 text-indigo-400 shrink-0" />}
                              {!link.allowDownload && <Eye className="w-2.5 h-2.5 text-amber-500 shrink-0" />}
                            </div>
                            <p className={`text-[8px] mt-0.5 ${isExpired || isExhausted ? 'text-rose-400' : 'text-slate-400'}`}>
                              {isExpired ? 'Expired' : isExhausted ? 'Limit reached' : `Expires ${new Date(link.expiresAt).toLocaleDateString()}`}
                              {' · '}
                              {link.downloadCount || 0}{link.maxDownloads ? `/${link.maxDownloads}` : ''} downloads
                            </p>
                          </div>
